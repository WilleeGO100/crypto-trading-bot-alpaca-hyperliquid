    ...
    def __init__(
        ...
        for idx, tok in enumerate(tokens):
            if isinstance(tok, dict):
                index_value = tok.get("index", idx)
                try:
                    index_value = int(index_value)
                except Exception:
                    index_value = idx
                tokens_by_index[index_value] = tok

        def _token_for_index(index: int) -> Optional[Dict[str, Any]]:
            if not isinstance(index, int):
                return None
            candidate = tokens_by_index.get(index)
            if candidate is None and 0 <= index < len(tokens):
                raw = tokens[index]
        ...
        for spot_info in spot_meta["universe"]:
            ...
            tokens_pair = spot_info.get("tokens", [])
            if not isinstance(tokens_pair, (list, tuple)) or len(tokens_pair) < 2:
                continue
            base, quote = tokens_pair[:2]
            base_info = _token_for_index(base)
            quote_info = _token_for_index(quote)
