...
Calculates gamma exposure levels from Deribit's API (authenticated when credentials are supplied).
...
def _rpc_post(method: str, params: Optional[Dict[str, Any]] = None, require_auth: bool = False) -> Dict[str, Any]:
    ...
    response = SESSION.post(f"{DERIBIT_BASE}/public/{method.split('/')[-1]}", json=payload, headers=headers, timeout=8)
...
def _ensure_auth() -> None:
    ...
    if not DERIBIT_API_KEY or not DERIBIT_API_SECRET:
        print("[DEBUG] Deribit API key/secret missing; skipping auth")
        return
    now = time.time()
    if _access_token and now < _token_expiry - 10:
        return
    payload = {
        "grant_type": "client_credentials",
        "client_id": DERIBIT_API_KEY,
        "client_secret": DERIBIT_API_SECRET,
    }
    print(
        "[DEBUG] Requesting Deribit token",
        f"grant_type={payload['grant_type']}",
        f"client_id_len={len(payload['client_id'])}",
        f"client_secret_len={len(payload['client_secret'])}",
    )
    try:
        result = _rpc_post("public/auth", payload, require_auth=False)
    except Exception as exc:
        print("[ERROR] Deribit auth exception", exc)
        _access_token = None
        _token_expiry = 0.0
        raise
    ...
    token_snippet = _access_token[:8] if _access_token else "None"
    print(
        "[DEBUG] Deribit auth succeeded",
        f"token_snippet={token_snippet}",
...
def fetch_btc_instruments() -> list[dict]:
    ...
    try:
        ...
        return _rpc_post("public/get_instruments", params)
...
def fetch_btc_ticker(instrument_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        return _rpc_post("public/ticker", params)
    except Exception:
        return None
...
def get_btc_spot() -> Optional[float]:
    ...
    try:
        ...
        result = _rpc_post("public/ticker", params)
